...
(async () => {
    ...
    app.get("/router", (req, res) => {
        ...
        if(page === "apply" && !req.user)
            return res.render(`${process.cwd()}/src/views/main/apply.notLogged.ejs`)
    ...
    app.post("/postApplication", async (req, res) => {
        if(!req.user) return res.send("You are not logged in... please refresh.")

        const data = req.body, coll = db.db.collection("applications");
        data["discord"] = req.user;
        data["discordID"] = (req.user as {id}).id

        if((await coll.findOne({discordID: (req.user as {id}).id}))) return res.send("You have already submitted an application, think this is mistake? Message Callum#6052");

        await coll.insertOne({
            ...data
        })
        
        return res.render(`${process.cwd()}/src/views/main/apply.submitted.ejs`)
    })
})();